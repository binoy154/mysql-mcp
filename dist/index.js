...
(function (PermissionLevel) {
    PermissionLevel["FULL_ACCESS"] = "full";
    PermissionLevel["READ_ONLY"] = "read_only";
    PermissionLevel["STAGING_WITH_CONFIRMATION"] = "staging_with_confirmation";
})(PermissionLevel || (PermissionLevel = {}));
// Tool categories for permission filtering
var ToolCategory;
(function (ToolCategory) {
    ToolCategory["READ_ONLY"] = "read_only";
    ToolCategory["WRITE_OPERATION"] = "write_operation";
    ToolCategory["ADMINISTRATIVE"] = "administrative";
...
class MySQLMCPServer {
    ...
    validateEnvironmentConnection(envName) {
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                if (currentEnv.permissions === PermissionLevel.STAGING_WITH_CONFIRMATION &&
                ...
                switch (name) {
                    case "switch_environment":
                        return await this.switchEnvironment(args?.environment);
                    case "connect_database":
                        return await this.connectDatabase(args);
                    case "list_databases":
                        return await this.listDatabases();
                    case "use_database":
                        return await this.useDatabase(args?.database);
                    case "list_tables":
                        return await this.listTables();
                    case "describe_table":
                        return await this.describeTable(args?.table);
                    case "select_query":
                        return await this.selectQuery(args?.query, args?.limit);
                    case "insert_data":
                        return await this.insertData(args?.table, args?.data);
                    case "update_data":
                        return await this.updateData(args?.table, args?.data, args?.where);
                    case "delete_data":
                        return await this.deleteData(args?.table, args?.where);
                    case "execute_query":
                        return await this.executeQuery(args?.query);
                    case "get_table_indexes":
                        return await this.getTableIndexes(args?.table);
                    case "get_foreign_keys":
                        return await this.getForeignKeys(args?.table);
                    ...
                    case "get_table_comments":
                        return await this.getTableComments(args?.table);
                    default:
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `✅ Successfully connected (and disconnected) from ${config.host}:${config.port}${config.database ? ` / ${config.database}` : ""}. Databases found: ${databases[0].length}.`,
                    },
                ],
    ...
    async listDatabases() {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Available databases:\n${rows.map(row => `- ${row.Database}`).join('\n')}`,
                },
            ],
        };
    }
    ...
    async selectQuery(query, limit = 100) {
        const connection = await this.ensureConnection();
    ...
    async insertData(table, data) {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Successfully inserted data into '${table}'. Insert ID: ${result.insertId}, Affected rows: ${result.affectedRows}`,
                },
            ],
        };
    }
    async updateData(table, data, where) {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Successfully updated '${table}'. Affected rows: ${result.affectedRows}`,
                },
            ],
        };
    }
    async deleteData(table, where) {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Successfully deleted from '${table}'. Affected rows: ${result.affectedRows}`,
                },
            ],
        };
    }
    ...
    async getDisabledToolResponse(toolName) {
        return {
            content: [
                {
                    type: "text",
